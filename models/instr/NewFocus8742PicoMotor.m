% © Copyright 2013-2015 Victor Bass, Shon Schmidt, and Jonas Flueckiger
% 
...
classdef NewFocus8742PicoMotor < InstrClass
    
    % NewFocus8742PicoMotor linear actuator user manual available online at:
    ...
    properties (Access = protected)
        ...
        angPos; % stage angle position
        fbrPos; % fiber angle position
        fullDistance;
    ...
    methods
        % constructor
        function self = NewFocus8742PicoMotor()
            self.Name = 'NewFocus8742PicoMotor';
...
% from n7744A code...
                    ...
                    self.Obj = icdevice('<<need New Focus driverLib>>', USB_address);
                    try
                        connect(self.Obj);
                    catch ME
                        rethrow(ME);
                    end
% end n7744a code copy...

            % tell user optical stage is connected
            if strcmp(self.Obj.Status, 'open')
                self.Connected = 1;
                disp('NewFocus8742PicoMotor connected');
            end
            ...
            %NewFocus8742PicoMotor initialization query
            ...
            %             self.send_command('1MO'); %Switch motor on
            %             self.send_command('1TS?');
            %             pause(2);
            %             %self.queryMotion();
            %
            %             %self.send_command('1TS?');
...
%                 end
        ...
        function self = move_stgangle(self, degrees)
            if self.Connected
                ...
                self.angPos = self.angPos + degrees*5000;
                self.Busy = 0;
                %                 else
                %                     msg = strcat(self.Name, ' not calibrated. Please calibrate before moving.');
                %                     error(msg);
...
%                 end
        ...
        function self = move_fbrangle(self, degrees)
            if self.Connected
                ...
                self.fbrPos = self.fbrPos + degrees*100;
                self.Busy = 0;
                %                 else
                %                     msg = strcat(self.Name, ' not calibrated. Please calibrate before moving.');
                %                     error(msg);
