% © Copyright 2013-2015 Jonas Flueckiger
% 
                    ...
                    try %maybe this needs to be taken one level up?
                        ...
                        if numel(absPos)~=3
                            try
								...
								disp(strcat('------- Try ', num2str(tries),'/5'));
								%self.Busy = 0;
                                %rethrow(ME);
                            end
                        end
                        if (tries==5)
							disp('CorvusEco Class problem: position is not returned propertly');
							disp('----------- Reached max number of attempts - abort '); 
                            self.Busy = 0; 
                            rethrow(ME);
                        end
                    end
