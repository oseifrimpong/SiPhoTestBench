...
classdef Laser_Agilent8164A < InstrClass
    ...
    methods
        ...
        function connect(self)
            if ~self.Connected
                self.Busy = 1;
                if self.Param.UseFastILEngine % start the server and engine
                    self.Param.UseFastILEngine
                    try
                        %Initialize N7700A Fast Spectral Loss
                        % Connect to Engine Manager
                        self.EngineMgr = actxserver('AgServerFSIL.EngineMgr');
                        % Create a new engine
                        self.Engine = self.EngineMgr.NewEngine;
                        % Load configuration file
                        self.Engine.LoadConfiguration = 'C:\Users\LuckyStrike\uwbiobench\drivers\shons_AgEngineFSIL.agconfig';
                        % Activate engine
                        % Activating Engine Connects to Hardware
                        self.Engine.LambdaZeroingMode = 1;
                ...
                else % use the VXI plug and play drivers
                    try
        ...
        function msg = off(self)
            if self.Lasing
                ...
                try
                    ...
                    msg = strcat(self.Name, ': Turned laser off.');
                catch ME
                    msg = ME.message;
                end
            else
        ...
        function msg = on(self)
            if ~self.Lasing
                ...
                try
                    ...
                    self.setLowSSE();
                    invoke(self.GroupObj.Tunablelasersources, 'settlslaserstate', self.Param.TunableLaserSlot, 1); % turn the tunable laser on, 
                    self.Lasing = self.laserIsOn();
                    msg = strcat(self.Name, ': Turning laser on.');
                catch ME
                    msg = ME.message;
                end
            else
        ...
        %% zero TLS
        function zeroTLS(self)
            % The TLS Zeroing (hp816x_TLS_zeroingAll) function starts lambda
            % zeroing operations on all back-loadable, 8196x and 8194x Tunable 
            % Laser modules within a 816x mainframe.
            invoke(self.GroupObj.Tunablelasersources,'tlszeroingall');
        end
