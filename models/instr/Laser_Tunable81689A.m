...
classdef Laser_Tunable81689A < Laser_Agilent8164A
    ...
    methods
        function self = Laser_Tunable81689A()
            ...
            self.Name = '8163A + 81689A TLS'; % name of the instrument
            ...
            self.StopWvl = 1575; % sweep wavelength (nm)
        ...
        function currentWvl = sweepNextStep(self)
            self.Busy = 1;
            if self.ReadyForSweep
                currentWvl = self.Param.Wavelength + self.StepWvl;
                self.setWavelength(currentWvl);
            end
        end
        
        function resetWvl(self)
            self.setWavelength(self.StartWvl);
        end
    end
end
