...
function cell_edit_cb(hObject, eventdata) 
end
...
%    disp(num2str(row)); % Shon debugging 12/16/2013
    column = eventdata.Indices(2);
%    disp(num2str(column)); % Shon debugging 12/16/2013
    switch column
        case 2 % select peaks
            % check for current location
                currentDevice = obj.devices.(obj.chip.CurrentLocation);
                table = get(obj.gui.panel(thisPanel).hAutoDeviceTable, 'Data');
                targetDevice = obj.devices.(table{row,1}); % col 1 has name
%                disp(targetDevice.Name); % Shon debugging 12/16/2013
            if strcmp(obj.chip.CurrentLocation, '') % no location specified
                ...
                if ~strcmp(currentDevice, targetDevice) % move to new device
                    currentDevice = obj.devices.(obj.chip.CurrentLocation);
                    moveToDevice(obj, currentDevice, targetDevice);  %scripted move function
                end
                
                fine_align(obj, 'panel', thisPanel);
                % Check number of detectors property in this device
                numDetectors = obj.instr.detector.getProp('NumOfDetectors');
                if isempty(targetDevice.getProp('NumOfDetectors'))
                    targetDevice.setProp('NumOfDetectors', numDetectors)
                end
                % Select Peak Popup
        ...
        case 3 % select for testing
            ...
            obj.devices.(table{row, 1}).setProp('Selected', selected);
            if selected
                targetDevice = obj.devices.(table{row,1});
                % Check number of detectors property in this device
                numDetectors = obj.instr.detector.getProp('NumOfDetectors');
                if isempty(targetDevice.getProp('NumOfDetectors'))
                    targetDevice.setProp('NumOfDetectors', numDetectors)
                end
                % Assign empty cell for peakLocations
                targetDevice.isPeak = zeros(1, numDetectors);
                targetDevice.PeakLocations = cell(1, numDetectors);
                targetDevice.PeakLocationsN = cell(1, numDetectors);
                targetDevice.PeakTrackWindows = cell(1, numDetectors);
                msg = targetDevice.checkPeakSelection();
                obj.msg(msg);
            end
    end
end
