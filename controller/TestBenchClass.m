...
classdef TestBenchClass < handle
    ...
    methods
        ...
        %% constructor
        function self = TestBenchClass(rootPath)
            if(nargin<1)
                rootPath = pwd;
            end
            ...
            ds = applicationDefaults(rootPath);
    ...
    methods % public
        ...
        function self = load_user(self, userID)
            try
                ...
                userfile = fullfile(self.AppSettings.path.userData, userID, 'AppSettings.mat');
        ...
        function self = new_user(self, userID)
            try
                ...
                try
                    ...
                    mkdir(fullfile(self.AppSettings.path.userData, userID, '')); % Under folder of User Name
                    mkdir(fullfile(self.AppSettings.path.userData, userID, 'coordinateFiles'));
                    mkdir(fullfile(self.AppSettings.path.userData, userID, 'recipeFiles'));
...
%                     username = fn(1:end-4); %get rid of .mat extension
        ...
        function checkPathInfo(self)
            ...
            for n = 1:length(pathNames)
                ...
                if ~strcmpi(thisPath(end), filesep)
    ...
    methods (Static)
        ...
        function initPanelBackgroundColor(panelH,backgroundColor)
            if(ishandle(panelH))
                contextmenuHandle = uicontextmenu; %('callback',@obj.contextmenu_mainaxes_callback);
                uimenu(contextmenuHandle,'Label','Change Color','separator','off','callback',{@TestBenchClass.changeBackgroundcolorCallback,panelH});
                set(panelH,'uicontextmenu',contextmenuHandle);
                if(nargin>1 && ~isempty(backgroundColor))
                    
                    set([panelH;
                        findall(panelH,'style','text')],'backgroundcolor',backgroundColor);
        ...
        function doContinue = confirmDlg(varargin)
            if(numel(varargin)<1 || isempty(varargin{1}))
                confirmMsg = 'Are you sure you want to adjust this value?';
            else
                confirmMsg = sprintf(varargin{:});
            end
            answer = questdlg(confirmMsg, ...
                'Warning', ...
                'Confirm', 'Cancel','Cancel');
            doContinue = strcmpi(answer,'Confirm');            
        end       
